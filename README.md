...
- 매서드(Method)
    ...
    - 매개변수 출력방식 -> 매개변수를 리턴값으로 사용하도록 대체해주는 방법(과도기적인 방법) , 키워드 out
    ```cs
...
- 일반화(Generic) 프로그래밍
    - 파이썬 - 변수에 제약사항 없음.
    - 타임의 제약을 해소하고자 만든 기능. ArrayList 등이 해결(단 , 박싱(언박싱)등 성능의 문제가 있음)
...
## 3일차
- 람다식
    - 익명 메서드를 만드는 방식 주에 하나 - delegate , lambda expression
    - 익명 메서드시 코딩량 줄여줌 , 프로퍼티 사용시에도 코딩량이 줄어듬
    - 익명 메서드 사용시 마다 대리자를 선언해야하기 때문에
        -Func , Action을 Ms에서 미리 만들어둠
    - 무명함수 : 람다식으로 만드는 익명 메소드 , 무명함수를 작성하기 위해서는 먼저 대리자로 무명함수의 
      모습을 결정 
...
- 파이썬 실행
    ...
    - 해당프로젝트에서 종속성, 마우스 오른쪽 버튼 > Nuget Package 관리
...
- 윈폼 UI 개발 + 파일, 스레드 
    ...
    - 그래픽 사용자 인터페이스를 만드는 방법
        1. Winfroms(Windows Forms)
        2. WPF(Windows Presentation Foundation) 
    - WYSIWYH(What You See Is What You Get) 방식의  GUI 프로그램 개발
...
- 토이 프로젝트 
    - 윈도우 탐색기 앱 종료
       음
    - 기능
        - 건강관리 healthCheck 데이터베이스에 등록한 Login 정보를 통해 로그인 할 수 있도록 구현 
        - 라디오버튼을 사용해서 클릭하면 여자와 남자의 수치가 변경됨
        - 데이터베이스를 이용하여 등록해둔 데이터를 클릭하면 각 텍스트박스에 맞게 정보가 들어가고 편하게 클릭하여 바로 실행
        - 리스트 박스를 사용하여 검사한 목록을 저장하고 한눈에 보기쉽게 정리
        - 검사 진행후 메시지박스를 통해서 순서대로 읽어볼수 있게 정보를 출력
    - 특징
        - 측정 값 분포도 위의 수치가 dB 연동된 여자 와 남자의 값대로 변경되어 알맞게 알아서 계산해주는 기능     
    - 보안
        - 관리 프로그램 목적으로 만들었지만 기능요소를 추가하고 사용목적으로 사용할 수 있도록 회원가입 기능을 추가 할 예정     
        - 스타트 폼을 만들어서 건강 기능 앱으로 좀 더 많은 기능을 추가 해야할것을 느낌
        - 회원관리 폼을 따로 만들어 추가 할 수 있는 기능
        - BMI 측정외 다른 건강 측정관리 폼을 더 여러개 만들어서 종합적으로 건강체크를 할 수 있는 폼을 만들예정     
    - 배운점
        - 윈폼으로 다양한 동작들을 배울 수 있어서 좋았고 생각한것 보다 더 많이 자세히 구현 할 수있어서 흥미를 많이 느꼈음
        - 이번 프로젝트 뿐만 아니라 더 재밋는 프로젝트를 만들어 보고 싶다는 욕심이 생겼음
        - 아직 모든 기능을 알지 못해서 아직 많이 부족하다고 생각했고 더 많이 배우고 싶다는 욕심이 생겼음  
    - 실행영상
