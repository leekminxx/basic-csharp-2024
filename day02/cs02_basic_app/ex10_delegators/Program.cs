namespace ex10_delegators
{
    ...
    class EventListener     // 이벤트가 발생하는 지 듣고 있는 객체
    {
        private string name;
        // 생성자
        ...
        {
            Console.WriteLine($"{name}.뭔 일이 발생했다!!! : {message}");
        }
    }
    ...
    class Sorting
    {
        ...
        // 내림차순 비교
        public int DescendingCompare(int a, int b)
        {
            if (a > b)
                return -1;
            else if (a == b)
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            // 대리자 체인!!! notifier의 EventOccured라는 대리자에 리스너 3개에서 일어날 수 있는 메서드를 연결
            // 일반적인 함수 호출에서 한 번에 여러 개의 함수 실행이 가능한가? 불가능하다!!! 
            ...
            notifier.EventOccured -= listener2.SomethingHappened;   // 리스너2의 함수는 실행하지마!
            ...
            sorting.BubbleSort(array, new Compare(sorting.DescendingCompare));
            ...
            Callback = new MyDelegate(calc.Plus);   // int a, int b가 아닌 Calculator 객체의 Plus() 메서드 전달
