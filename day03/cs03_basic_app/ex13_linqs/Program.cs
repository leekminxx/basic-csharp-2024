    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            Product[] arrProducts =
            {
                ...
                new Product { Title = "엽기적인그녀", Star = "전지현" },
            ...
            profiles.Sort(
                (profiles1, profiles2) =>
                {
                    return profiles1.Height - profiles2.Height;
                });
            ...
            var groupProfiles = from p in arrProFiles
                                orderby p.Height descending  //작은사람부터 큰사람 반대로 뽑고싶으면 descending
            ...
            // LINQ JOIN
            var innerJoinsResult = from pf in arrProFiles
            ...
            foreach (var item in innerJoinsResult)
            {
            ...
            var outerJoinResult = from pf in arrProFiles
                                  join pr in arrProducts
                                  on pf.Name equals pr.Star 
                                  into ps // 외부조인시 내부조인 LINQ에 추가되는부분
                                  from pr in ps.DefaultIfEmpty(new Product() { Title = "작품춤없음" }) // 외부조인시 내부조인 LINQ 에 추가되는 부분 
                                  select new
                                  {
                                      Name = pf.Name,
                                      Work = pr.Title,
